...
export default function PlanView({ plan }: { plan: Plan }) {
  const summaries = computeWeekSummaries(plan.activities);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex mb-2">
        <div className="min-w-56 pr-4 text-sm font-semibold text-zinc-500">
          Week summary
        </div>
        <div className="grid grid-cols-7 flex-1 gap-px">
          {DAYS.map((day) => (
            <div
              key={day}
              className="text-sm font-semibold text-zinc-500 px-2 pb-1"
            >
              {day}
            </div>
          ))}
      ...
      {summaries.map((summary, i) => {
        ...
        return (
          <div key={summary.startDate}>
            {isBlockBoundary && <div className="h-6 bg-zinc-100 my-2" />}
            <WeekRow
              summary={summary}
